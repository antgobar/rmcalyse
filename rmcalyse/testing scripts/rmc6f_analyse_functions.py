...
def av_unit_cell(orthonormal_positions, atom, max_param):
    ...
    ##to_ave = [cell_param_list[dist][10] for dist, i in enumerate(cell_param_list)]
...
def iad_hist(i_a_dist_list, atomA, atomB, bin_start, bin_end, bin_size):
    
    hist_list = []
    
    binning = np.arange(bin_start, bin_end, bin_size).tolist()
    
    for i, distance in enumerate(i_a_dist_list):
        hist_list.append(i_a_dist_list[i][10])

    ## Does same as above but list comprehension
    ##hist_list = [i_a_dist_list[n][10] for n, atom in enumerate(i_a_dist_list)]
    
    plt.hist(hist_list, binning)
    plt.show()
...
###=====================================================================
###---------------------------------------------------------------------
###                     AVERAGE SUPERCELL "GRID" NEEDS COMPLETING
